...
const TicketForm = () => {
  ...
  // Select specific fields from store to prevent unnecessary re-renders
  ...
  // Read the saved data ONCE for initialization without subscribing
  ...
  // Watch for changes and save to store automatically
  ...
  const renderFormField = (field: any) => {
    ...
    switch (field.type) {
      case 'select':
        return (
          <select
            {...register(field.name as keyof TicketFormData, registerOptions)}
        ...
        return (
          <label className="flex items-start space-x-3 cursor-pointer">
            <input
              type="checkbox"
              {...register(field.name as keyof TicketFormData, registerOptions)}
              className="mt-1 w-5 h-5 text-yellow-600 rounded focus:ring-yellow-500 border-gray-300 dark:border-red-900"
            />
            <span className={`text-sm ${hasError ? 'text-red-600' : 'text-gray-700 dark:text-gray-300'}`}>
              {field.label}
            </span>
          </label>
        );
        ...
        >
          ...
          <div className="mb-10">
            <div className="flex items-center justify-between mb-4 px-4">
              {[1, 2, 3, 4].map(step => (
                <div key={step} className="flex flex-col items-center z-10">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg transition-all duration-500 border-4 ${
                    step === currentStep 
                      ? 'bg-yellow-500 text-red-900 border-yellow-300 shadow-lg scale-110' 
                      : step < currentStep 
                        ? 'bg-green-600 text-white border-green-500'
                        : 'bg-gray-200 text-gray-400 border-gray-300 dark:bg-red-950 dark:text-red-700 dark:border-red-900'
                  }`}>
                  ...
                  <span className={`text-xs mt-2 font-bold uppercase tracking-wider transition-colors duration-300 ${
                    step === currentStep 
                      ...
                      : 'text-gray-400 dark:text-red-800'
                  }`}>
                    {step === 1 ? 'Bio' : step === 2 ? 'Church' : step === 3 ? 'Health' : 'Consent'}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="relative w-full h-1 bg-gray-200 dark:bg-red-950 rounded-full -mt-8 mb-8 z-0 mx-4 max-w-[calc(100%-2rem)]">
              <motion.div 
                className="bg-gradient-to-r from-green-600 via-yellow-500 to-yellow-400 h-full rounded-full shadow-md"
                initial={{ width: "0%" }}
              ...
              <div className="flex justify-between mt-12 pt-8 border-t border-gray-200 dark:border-white/10">
                {currentStep > 1 ? (
                  <button
                    ...
                    className="px-8 py-3 border border-gray-300 dark:border-white/20 text-gray-600 dark:text-white/70 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors font-bold disabled:opacity-50"
                  >
                    ← BACK
                  </button>
                ) : <div></div>}
                ...
                {currentStep < 4 ? (
                  <button
                    type="button"
                    onClick={nextStep}
                    className="btn-primary px-10 py-3 rounded-xl shadow-lg flex items-center gap-2"
                  >
                    NEXT STEP →
                  </button>
                ) : (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="btn-primary px-12 py-3 rounded-xl shadow-xl disabled:opacity-70 disabled:grayscale flex items-center gap-2"
                  >
                    {isSubmitting ? (
                      <>
                        <svg className="animate-spin -ml-1 h-5 w-5 text-red-900" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
