...
const CoordinatorDashboard = () => {
  ...
  const handleBulkDownload = () => {
    ...
    const doc = new jsPDF();
    let yPos = 20;
    doc.setFontSize(16); doc.text(`RCCG R63 CAMP TICKETS - ${user?.province} (Page ${currentPage})`, 105, 15, { align: "center" });
    ...
    doc.save(`${user?.province}_Tickets_Page${currentPage}.pdf`); toast.success("Tickets PDF downloaded successfully!");
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="card p-6 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm">
              ...
              <div className="flex justify-between text-xs font-medium pt-4 border-t border-gray-100 dark:border-white/5">
                {/* Note: Counts below are filtered by current view if we derived them from myTickets. 
                    Ideally we should fetch global stats separately. For now, showing 'Displaying X' might be better or hiding them.
                    Let's hide specific counts for now as we don't have aggregated stats API yet */}
          ...
          <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm">
            <div className="p-4 border-b border-gray-200 dark:border-white/10 flex flex-col md:flex-row gap-4 justify-between items-center bg-gray-50 dark:bg-white/5">
              <div className="relative w-full md:w-96">
                ...
                <input type="text" placeholder="Search name or Ticket ID..." value={searchTerm} onChange={handleSearch} className="w-full pl-10 pr-4 py-2 bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-lg outline-none focus:ring-2 focus:ring-yellow-500 dark:text-white" />
              </div>
