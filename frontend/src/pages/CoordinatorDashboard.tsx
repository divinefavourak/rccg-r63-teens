...
const CoordinatorDashboard = () => {
  ...
  // Helper to normalize strings for comparison (removes spaces, underscores, lowercase)
  const normalize = (str: string) => str ? str.toLowerCase().replace(/[\s_]/g, '') : '';
  ...
  const fetchMyData = async () => {
    ...
    try {
      ...
      const filtered = allTickets.filter(t => {
        const ticketProv = normalize(t.province);
        // Match if strings are identical OR one contains the other
        return ticketProv === userProv || ticketProv.includes(userProv) || userProv.includes(ticketProv);
      });
  ...
  // ... (Keep handleDownloadCSV and handleBulkDownload exactly as they were) ...
  // Re-pasting them for completeness if you copy-paste the whole file
  ...
  const handleBulkDownload = () => {
    ...
    const doc = new jsPDF();
    let yPos = 20;
    doc.setFontSize(16); doc.text(`RCCG R63 CAMP TICKETS - ${user?.province}`, 105, 15, { align: "center" });
    ...
    myTickets.forEach((ticket, index) => {
      ...
      doc.text(`Name: ${ticket.fullName}`, 15, yPos + 20); doc.text(`Category: ${ticket.category.toUpperCase()}`, 15, yPos + 30);
      doc.text(`Gender: ${ticket.gender}`, 100, yPos + 20); doc.text(`Status: ${ticket.status.toUpperCase()}`, 100, yPos + 30);
      doc.setTextColor(100); doc.text("Present this ID at the gate.", 15, yPos + 40);
      yPos += 55;
    });
    doc.save(`${user?.province}_Tickets.pdf`); toast.success("Tickets PDF downloaded successfully!");
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="card p-6 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div><p className="text-xs font-bold uppercase text-gray-500 dark:text-gray-400">Total Registered</p><p className="text-2xl font-black">{stats.total}</p></div>
              </div>
            ...
            <Link to="/coordinator/bulk-register" className="card p-6 bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-500/20 rounded-2xl shadow-sm hover:shadow-md transition-all group cursor-pointer">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-yellow-200 dark:bg-yellow-500/20 flex items-center justify-center text-yellow-700 dark:text-yellow-400 text-xl group-hover:scale-110 transition-transform"><FaLayerGroup /></div>
            ...
            <Link to="/coordinator/single-register" className="card p-6 bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-500/20 rounded-2xl shadow-sm hover:shadow-md transition-all group cursor-pointer">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-green-200 dark:bg-green-500/20 flex items-center justify-center text-green-700 dark:text-green-400 text-xl group-hover:scale-110 transition-transform"><FaUserPlus /></div>
          ...
          <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm">
            <div className="p-4 border-b border-gray-200 dark:border-white/10 flex flex-col md:flex-row gap-4 justify-between items-center bg-gray-50 dark:bg-white/5">
              <div className="relative w-full md:w-96">
                ...
                <input type="text" placeholder="Search name or Ticket ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-lg outline-none focus:ring-2 focus:ring-yellow-500 dark:text-white" />
              </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                  {loading ? (
                    ...
                    <tr><td colSpan={6} className="p-12 text-center text-gray-500 flex flex-col items-center"><FaUsers className="text-4xl mb-2 opacity-20" /><p>No candidates found for {user?.province}</p><Link to="/coordinator/single-register" className="text-yellow-600 dark:text-yellow-400 underline mt-2">Start Registration</Link></td></tr>
                  ) : (
